...
const _get = Joi.object().keys({
	...
	status: Joi.string().valid('public', 'draft', 'expired').optional(),
...
const _add = Joi.object().keys({
	...
	status: Joi.string().valid('public', 'draft', 'expired').default('draft').optional(),
	...
	career: Joi.array().items(Joi.object().keys({
	...
	academic_level: Joi.array().items(Joi.object().keys({
			id: Joi.string().allow(null, '').max(36).optional(),
			studyNow: Joi.boolean().allow(null, '').default(true).optional(),
	...
	jobposition: Joi.string().optional().allow(null, ''),
	positionalt: Joi.string().optional().allow(null, ''),
	postionmatching: Joi.array().items(Joi.string()).default([]).optional(),
	businessturn: Joi.string().optional().allow(null, ''),
	isBranch: Joi.boolean().default(false).optional(),
	locationState: Joi.string().optional().default('public').valid('public', 'private').allow('', null),
	experience: Joi.number().optional().allow('', null),
	interviewPlace: Joi.string().optional().valid('branch', 'office').allow('', null),
});
...
const _update = Joi.object().keys({
	...
	company_state: Joi.string().valid('public', 'confidential').default('public').optional(),
	avatar: Joi.string().max(50).allow(null, '').default(null).optional(),
	title: Joi.string().min(6).max(250).optional(),
	availability: Joi.string().valid('freelance', 'practice', 'temporal', 'full', 'part', 'vacacionista').default('temporal').optional(),
	career: Joi.array().items(Joi.object().keys({
			id: Joi.string().min(6).max(36).allow(null, '').required(),
			name: Joi.string().min(2).max(150).allow(null, '').optional(),
