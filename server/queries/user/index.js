...
const refresh = req => new Promise((resolve, reject) => {
	console.log("RESOLVED FROM REFRESH TOKEN")
...
const login = (r, conn, params) => new Promise((resolve, reject) => {
	...
	r
		...
		.run(conn, async (err, results) => {
			...
			results.toArray((err, rows) => {
				...
				/** Prepare token */
				user.token = JWT.sign(user, config.get('/api_secret'), {
					algorithm: 'HS256',
					expiresIn: "1d"
				})
...
const update = (r, conn, id, payload) =>
