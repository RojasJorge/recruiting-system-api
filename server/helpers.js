...
const map_filters = (req, doc) => {
	 ...
	 * Calculate 1 month
	 */
	const amonth = moment(new Date()).subtract(1, 'month').format('YYYY-M-D')

	if (status) {
		let publish = doc('status').downcase().eq(status)

		if (status === 'public') {
			publish = publish.and(doc('created_at').date().gt(new Date(amonth)))
		}
